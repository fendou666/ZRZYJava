...
public class Ch10_9 {
	...
	static int LinSum(int row, int col) 					//统计四周细胞数量 
	{
	   ...
	   if (cell[row][col] == ALIVE) 			//当前单元格为活细胞，自己去掉 
	   {
		  count--; 
	   }
	...
	static void OutCell() 								//输出显示细胞状态 
	{
	   ...
	   System.out.printf("\n细胞状态\n");
	   System.out.printf("┌");	 
	   for (col = 0; col < COLLEN -1; col++)			//最上面一行
	   {
		   System.out.printf("─┬");
	   }
	   System.out.printf("─┐\n");
		...
		for (row = 0; row < ROWLEN; row++)
		  {
			 for (col = 0; col < COLLEN; col++)
			 {
				switch (LinSum(row, col)) 		//统计四周活细胞数量 
				{
				case 2:
	      ...
	      for (row = 0; row < ROWLEN; row++) //设置下一个状态
		  {
	    	  for (col = 0; col < COLLEN; col++)
			  {
		  ...
		  for (row = 0; row < ROWLEN; row++)//统计活细胞数量
		  {
		      for (col = 0; col < COLLEN; col++)
			  {
